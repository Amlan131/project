    ...
    try:
        ...
        for chunk in discourse_chunks:
            try:
                ...
                logger.error(
                    f"Error processing discourse chunk {chunk['id']}: {e}")
        ...
        for chunk in markdown_chunks:
            try:
                ...
                logger.error(
                    f"Error processing markdown chunk {chunk['id']}: {e}")
...
async def generate_answer(question, relevant_results, max_retries=2):
    ...
    while retries < max_retries:
        try:
            logger.info(
                f"Generating answer for question: '{question[:50]}...'")
            context = ""
    ...
    try:
        ...
        if len(parts) > 1:
            ...
            for line in source_lines:
                ...
                url_match = re.search(
                    r'URL:\s*\[(.*?)\]|url:\s*\[(.*?)\]|\[(http[^\]]+)\]|URL:\s*(http\S+)|url:\s*(http\S+)|(http\S+)', line, re.IGNORECASE)
                text_match = re.search(
                    r'Text:\s*\[(.*?)\]|text:\s*\[(.*?)\]|[""](.*?)[""]|Text:\s*"(.*?)"|text:\s*"(.*?)"', line, re.IGNORECASE)
                ...
                if url_match:
                    ...
                    if text_match:
                        # Find the first non-None group from the regex match
                        text_value = next(
                            (g for g in text_match.groups() if g), "")
                        if text_value:
        ...
        logger.info(
            f"Parsed answer (length: {len(answer)}) and {len(links)} sources")
...
async def query_knowledge_base(request: QueryRequest):
    try:
        ...
        try:
            ...
            if not result["links"]:
                logger.info(
                    "No links extracted, creating from relevant results")
                ...
                for res in relevant_results[:5]:  # Use top 5 results
                    ...
                    if url not in unique_urls:
                        ...
                        snippet = res["content"][:100] + \
                            "..." if len(
                                res["content"]) > 100 else res["content"]
...
async def health_check():
    try:
        ...
        return JSONResponse(
            status_code=500,
            content={"status": "unhealthy", "error": str(
                e), "api_key_set": bool(API_KEY)}
        )
